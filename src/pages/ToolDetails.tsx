...
import { Star, MapPin, User, CheckCircle, ArrowLeft, Heart, Loader2 } from 'lucide-react';
...
const ToolDetails = () => {
  ...
  const fetchReviews = async (page: number = 1) => {
    ...
    try {
      ...
      const reviewsData = await toolsService.getToolReviews(id, page, reviewsPerPage);
  ...
  const calculateDisplayPrice = (basePrice: number | null | undefined): number => {
  ...
  const categoryName = toolDetails.category?.displayName || "Unknown Category";
  const subcategoryName = toolDetails.subcategory?.displayName || "Unknown Subcategory";
  const ownerName = `${toolDetails.owner?.firstName || ""} ${toolDetails.owner?.lastName || ""}`.trim() || "Unknown Owner";
  const displayPrice = calculateDisplayPrice(toolDetails.basePrice);
  const primaryPhotoUrl = getPrimaryPhotoUrl(toolDetails);
  const allPhotoUrls = getAllPhotoUrls(toolDetails);
  const feeAmount = (toolDetails.basePrice || 0) * 0.054;
              ...
              {language === 'ar' ? (
                ...
                <div >
                  <div className="flex items-center gap-2">
                    {toolDetails.owner?.verifiedEmail && (
                ...
                <Avatar className="h-16 w-16">
                  <AvatarImage src={toolDetails.owner?.profilePicture || "/placeholder.svg"} alt={ownerName} />
                  <AvatarFallback>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div>
              <div className={"flex items-center gap-2 mb-4 "+ (language === 'ar' ? '[direction:ltr]' : '')}>
                ...
                <Badge className="bg-green-500">{tool.condition || t('add_tool.condition_new')}</Badge>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="grid grid-cols-2 gap-4 ">
                  <div>
                    ...
                    <span className="ml-2 font-medium">{toolDetails.brand || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">{t('tools.model')}:</span>
                    <span className="ml-2 font-medium">{toolDetails.model || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">{t('tools.year_of_purchase')}:</span>
                    <span className="ml-2 font-medium">{toolDetails.purchaseYear || 'N/A'}</span>
                  </div>
              ...
              <div className={"flex items-center gap-4 mb-6 "+ (language === 'ar' ? '[direction:ltr]' : '')}>
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-gray-500">({tool.reviewCount || 0} avis)</span>
                </div>
              </div>

              <div className="bg-accent/5 rounded-lg p-6 mb-6">
                <div className="text-3xl font-bold text-accent mb-2">
                  {(displayPrice || 0).toFixed(1)}€<span className="text-lg font-normal text-gray-600">/{t('tools.day')}</span>
                </div>
                <div className="text-sm text-gray-600 mb-4">
                  {t('tools.fees_and_taxes')} : {(feeAmount || 0).toFixed(1)} € (5,4% {t('tools.of')} {toolDetails.basePrice || 0} € {t('tools.charged')})
                </div>
                <div className="text-sm text-gray-600 mb-4">
                  {t('tools.deposit')}: {toolDetails.deposit || 0}€ {t('tools.refunded')}
                </div>
            ...
            <div>
              ...
              <Card className="mb-6">
                <CardContent className="p-6">
                  ...
                  <p className="text-gray-700">{toolDetails.ownerInstructions || 'Aucune instruction spécifique fournie.'}</p>
                </CardContent>
              </Card>
                  ...
                  {totalReviewPages > 1 && (
                    <Pagination>
                      <PaginationContent className={language === 'ar' ? "[direction:ltr]" : ''}>
                        <PaginationItem>
                          <PaginationPrevious 
                            onClick={() => handleReviewPageChange(Math.max(1, currentReviewPage - 1))}
                        ...
                        <PaginationItem>
                          <PaginationNext 
                            onClick={() => handleReviewPageChange(Math.min(totalReviewPages, currentReviewPage + 1))}
