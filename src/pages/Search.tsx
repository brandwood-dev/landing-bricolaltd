...
import { Search as SearchIcon, MapPin, Star, Filter, Heart, Loader2, X, RefreshCw } from 'lucide-react';
...
const Search = () => {
  ...
  const [sortBy, setSortBy] = useState<'title' | 'basePrice' | 'rating' | 'createdAt'>('createdAt');
  ...
  const fetchTools = useCallback(async (isRealTimeSearch = false) => {
    if (isRealTimeSearch) {
      setIsSearching(true);
    } else {
      setLoading(true);
    ...
    try {
      ...
      const filters: ToolFilters = {
        page: currentPage,
        limit: itemsPerPage,
        search: isRealTimeSearch ? debouncedSearchQuery : searchQuery || undefined,
    ...
    setTools(response.data || []);
      ...
      if (filters.subcategoryId) newParams.set('subcategoryId', filters.subcategoryId);
  ...
  }, [debouncedSearchQuery, debouncedLocationQuery, selectedCategory, selectedSubCategory, priceRange, sortBy, sortOrder, currentPage, itemsPerPage, searchQuery, locationQuery, navigate, toast]);
  ...
  useEffect(() => {
    if (debouncedSearchQuery !== searchQuery || debouncedLocationQuery !== locationQuery) {
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1">
              ...
              <Card className={`lg:block ${filtersVisible ? 'block' : 'hidden'}`}>
            ...
            <div className="lg:col-span-3">
              <div className={"flex justify-between items-center mb-6 "+ (language === 'ar' ? ' [direction:ltr] ' : '')}>
                <div className="flex items-center gap-3">
                  <div>
                    <h1 className={"text-2xl font-bold " + (language === 'ar' ? '[direction:rtl]' : '')}>{totalItems} {t('catalog_section.title')}</h1>
                    <div className="flex items-center gap-2 mt-1">
                      {selectedCategory !== 'all' && (
                        <p className="text-gray-600">
                          {t('catalog_section.category')}: {categories.find(cat => cat.id === selectedCategory)?.displayName || selectedCategory}
                        </p>
                      )}
                      {isSearching && (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {(searchQuery || locationQuery || selectedCategory !== 'all' || selectedSubCategory !== 'all') && (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={clearAllFilters}
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm mt-1">Veuillez réessayer plus tard</p>
                    </div>
                ...
                <div className="text-center py-12">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div>
                      <p className="text-gray-600 text-lg font-medium">{t('search.noResults')}</p>
                      <p className="text-gray-400 text-sm mt-1">{t('search.tryDifferentCriteria')}</p>
                    </div>
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div className="text-lg font-bold text-primary">
                            {calculateDisplayPrice(tool.basePrice)}€<span className="text-sm font-normal text-gray-500">/{t('tools.day')}</span>
                          </div>
                          <div className="text-sm text-gray-500">
                            {t('catalog_section.by')} {tool.owner?.firstName || 'Propriétaire'}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {isAuthenticated && (
                            <Button
                              ...
                              className="p-2"
                            >
                              <Heart className={`h-4 w-4 ${isFavorite(tool.id) ? 'fill-red-500 text-red-500' : ''}`} />
                            </Button>
                          )}
                        </div>
                      </div>
