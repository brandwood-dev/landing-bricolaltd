...
class CurrencyService {
  ...
  private isValidCache(timestamp: number): boolean {
    ...
    console.log(
      `⏰ [CurrencyService] Cache age: ${age}ms, Valid: ${isValid} (Duration: ${this.CACHE_DURATION}ms)`
    )
    return isValid
  }
  ...
  private async fetchWithCache<T>(
    ...
    try {
      ...
      console.log(
        `📡 [CurrencyService] API response received in ${requestTime}ms`
      )
      console.log(
        `📊 [CurrencyService] Response status: ${response.status} ${response.statusText}`
      )

      if (!response.ok) {
        console.error(
          `❌ [CurrencyService] HTTP error! status: ${response.status}`
        )
      ...
      if (!result || typeof result !== 'object') {
        console.error(
          `❌ [CurrencyService] Invalid response structure:`,
          result
        )
      ...
      if (!result.data) {
        console.error(
          `❌ [CurrencyService] Missing data field in response:`,
          result
        )
  ...
  async getExchangeRate(
    ...
    console.log(
      `💱 [CurrencyService] Getting exchange rate: ${fromCurrency} → ${toCurrency}`
    )
    try {
      const result = await this.fetchWithCache<ExchangeRateResponse>(
        '/exchange-rates',
        {
      ...
      console.error(
        `❌ [CurrencyService] Failed to get exchange rate ${fromCurrency} → ${toCurrency}:`,
        error
      )
      throw error
    }
  ...
  async getBulkExchangeRates(
    ...
    console.log(
      `📊 [CurrencyService] Getting bulk exchange rates for base: ${baseCurrency}`
    )
    try {
      const result = await this.fetchWithCache<BulkExchangeRateResponse>(
        '/exchange-rates/bulk',
        {
      ...
      console.log(
        `📊 [CurrencyService] Number of rates received: ${
          result.rates ? Object.keys(result.rates).length : 0
        }`
      )
      return result
    } catch (error) {
      console.error(
        `❌ [CurrencyService] Failed to get bulk exchange rates for ${baseCurrency}:`,
        error
      )
      throw error
    }
  ...
  async convertCurrency(
    ...
    console.log(
      `🔄 [CurrencyService] Converting ${amount} ${fromCurrency} → ${toCurrency}`
    )
    try {
      const result = await this.fetchWithCache<ConvertedPrice>(
      ...
      console.error(
        `❌ [CurrencyService] Failed to convert ${amount} ${fromCurrency} → ${toCurrency}:`,
        error
      )
      throw error
    }
  ...
  clearCache(): void {
    ...
    this.cache.clear()
    console.log(
      `🗑️ [CurrencyService] Cache cleared (${cacheSize} entries removed)`
    )
  }
