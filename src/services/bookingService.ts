...
export class BookingService {
  ...
  async getBooking(id: string): Promise<Booking> {
    try {
      console.log(
        '[bookingService.getBooking] 🔍 START fetching booking id=',
        id,
      )
      const response = await api.get<ApiResponse<Booking>>(`/bookings/${id}`)
      console.log(
        '[bookingService.getBooking] 📦 Raw axios response.data keys:',
        Object.keys(response?.data || {}),
      )
      const responseData = response.data as any

      // Detect envelope format: { success: true, data: booking } vs direct booking object
      let booking: Booking
      if (
        responseData?.data &&
        ...
        console.log(
          '[bookingService.getBooking] ✅ Detected standard envelope: response.data.data',
        )
      } else if (responseData?.id || responseData?.toolId) {
        booking = responseData as Booking
        console.log(
          '[bookingService.getBooking] ⚠️ Detected DIRECT booking object (no envelope)',
        )
      } else {
        booking = (responseData?.data ?? responseData) as Booking
        console.log(
          '[bookingService.getBooking] ⚠️ Falling back to best-effort extraction',
        )
      }

      console.log(
        '[bookingService.getBooking] 🔎 Extracted booking sanity check:',
        {
          hasId: !!booking?.id,
          hasTool: !!booking?.tool,
          hasToolId: !!booking?.toolId,
          hasCategory: !!(booking as any)?.tool?.category,
          hasSubcategory: !!(booking as any)?.tool?.subcategory,
        },
      )
      ...
      console.error(
        '[bookingService.getBooking] ❌ ERROR:',
        error?.message || error,
      )
  ...
  async getOwnerBookings(
    ...
    try {
      ...
      const queryString = params.toString()
      const url = queryString
        ? `/bookings/user/owner/${userId}?${queryString}`
        : `/bookings/user/owner/${userId}`
      const response = await api.get<ApiResponse<Booking[]>>(url)
      return { data: response.data.data }
    } catch (error: any) {
      throw new Error(
        extractErrorMessage(error, 'Failed to fetch owner bookings'),
      )
    }
  }
  ...
  async updateBookingStatus(
    ...
    statusOrData:
      | Booking['status']
      | { status?: Booking['status']; pickupTool?: boolean },
  ): Promise<Booking> {
    try {
      const updateData =
  ...
  async validateBookingCode(
    ...
    try {
      ...
      throw new Error(
        extractErrorMessage(error, 'Failed to validate booking code'),
      )
    }
  }
  ...
  async checkToolAvailability(
    ...
    try {
      const response = await api.post<ApiResponse<BookingAvailabilityCheck>>(
      ...
      throw new Error(
        extractErrorMessage(error, 'Failed to check availability'),
      )
    }
  }
  ...
  async getToolUnavailableDates(toolId: string): Promise<string[]> {
    try {
      ...
      throw new Error(
        extractErrorMessage(error, 'Failed to fetch unavailable dates'),
      )
    }
  }
  ...
  async calculateBookingPricing(
    ...
    try {
      const response = await api.post<ApiResponse<BookingPricing>>(
        '/bookings/calculate-pricing',
        {
          toolId,
          startDate,
          endDate,
  ...
  async confirmToolReturn(id: string, notes?: string): Promise<Booking> {
    try {
      ...
      throw new Error(
        extractErrorMessage(error, 'Failed to confirm tool return'),
      )
    }
  }
  ...
  async getBookingStats(): Promise<BookingStats> {
    try {
      const response =
        await api.get<ApiResponse<BookingStats>>('/bookings/stats')
      return response.data.data
    } catch (error: any) {
      throw new Error(
        extractErrorMessage(error, 'Failed to fetch booking statistics'),
      )
    }
  }
  ...
  async reportPickup(
    ...
    try {
      ...
      const response = await api.post<ApiResponse<Booking>>(
        `/bookings/${bookingId}/report-pickup`,
        formData,
        {
          headers: {
      ...
      throw new Error(
        extractErrorMessage(error, 'Failed to report pickup issue'),
      )
    }
  }
}
