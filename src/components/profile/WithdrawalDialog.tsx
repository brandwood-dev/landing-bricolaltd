...
const WithdrawalDialog: React.FC<WithdrawalDialogProps> = ({
  ...
  const [step, setStep] = useState<
    'amount' | 'method' | 'details' | 'confirmation'
  >('amount')
  ...
  const maxAmountGBP =
    (userBalance as any).availableBalance ?? userBalance.balance ?? 0
  const ratesReady =
    currency.code === 'GBP' || getInstantRate('GBP', currency.code) !== null
  const minAmountInSelected =
    currency.code === 'GBP'
      ? minAmountGBP
      : convertInstantly(minAmountGBP, 'GBP', currency.code)
  const maxAmountInSelected =
    currency.code === 'GBP'
      ? maxAmountGBP
      : convertInstantly(maxAmountGBP, 'GBP', currency.code)
  ...
  useEffect(() => {
    ...
    void refreshRates('user_currency_change')
  }, [open, currency.code, refreshRates])

  const validateAmount = (amountSelected: number) => {
    if (currency.code !== 'GBP' && !ratesReady) {
      setErrors({ amount: t('pricing.load_error') })
      return false
    }
    ...
    const amountGBP =
      currency.code === 'GBP'
        ? amountSelected
        : convertInstantly(amountSelected, currency.code, 'GBP')
    ...
    const validation = walletService.validateWithdrawalAmount(
      amountGBP,
      maxAmountGBP,
    )
  ...
  const handleMethodChange = (
    method: 'bank_transfer' | 'stripe_connect' | 'card_payout',
  ) => {
    ...
    try {
      const amountGBP =
        currency.code === 'GBP'
          ...
          : convertInstantly(withdrawalData.amount || 0, currency.code, 'GBP')
      ...
      const createdTx = await walletService.createWithdrawal(
        userId,
        payload as any,
      )
    ...
    <div className='space-y-6'>
      <div className='text-center'>
        ...
        <h3 className='text-lg font-semibold mb-2'>
          {t('wallet.dialog.amount_step.title')}
        </h3>
        <p className='text-sm text-gray-600'>
          {t('wallet.dialog.amount_step.subtitle').replace(
            '{amount}',
            formatInstantPrice(maxAmountGBP, 'GBP', currency.code),
          )}
        </p>
      </div>
      ...
      <div className='space-y-4'>
        <div>
          <Label htmlFor='amount'>
            {t('wallet.dialog.amount_step.label').replace(
              '{symbol}',
              currency.symbol,
            )}
          </Label>
          <Input
            ...
            placeholder={t('wallet.dialog.amount_step.placeholder').replace(
              '{amount}',
              formatInstantPrice(minAmountGBP, 'GBP', currency.code),
            )}
        ...
        <Alert>
          ...
          <AlertDescription>
            {t('wallet.dialog.amount_step.alert').replace(
              '{amount}',
              formatInstantPrice(minAmountGBP, 'GBP', currency.code),
            )}
          </AlertDescription>
  ...
  const renderMethodStep = () => (
    <div className='space-y-6'>
      <div className='text-center'>
        ...
        <h3 className='text-lg font-semibold mb-2'>
          {t('wallet.dialog.method_step.title')}
        </h3>
        <p className='text-sm text-gray-600'>
          {t('wallet.dialog.method_step.subtitle')}
        </p>
      </div>
            ...
            <div className='flex items-center space-x-3'>
              ...
              <div>
                <div className='font-medium'>
                  {t('wallet.dialog.method_step.bank_transfer')}
                </div>
                <div className='text-sm text-gray-500'>
                  {t('wallet.dialog.method_step.bank_delay')}
                </div>
              </div>
            </div>
          </CardContent>
  ...
  const renderDetailsStep = () => (
    <div className='space-y-6'>
      <div className='text-center'>
        ...
        <h3 className='text-lg font-semibold mb-2'>
          {t('wallet.dialog.details_step.title')}
        </h3>
        <p className='text-sm text-gray-600'>
          {t('wallet.dialog.details_step.subtitle')}
        </p>
      </div>
        ...
        <div className='space-y-4'>
          <div>
            <Label htmlFor='accountHolderName'>
              {t('wallet.dialog.details_step.account_name')}
            </Label>
            <Input
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                handleBankDetailsChange('iban', e.target.value.toUpperCase())
              }
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                handleBankDetailsChange('bic', e.target.value.toUpperCase())
              }
      ...
      {withdrawalData.paymentMethod === 'stripe_connect' && (
        <div>
          <Label htmlFor='stripeAccountId'>
            {t('wallet.dialog.details_step.stripe_account')}
          </Label>
          <Input
            id='stripeAccountId'
  ...
  const renderConfirmationStep = () => (
    <div className='space-y-6'>
      <div className='text-center'>
        ...
        <h3 className='text-lg font-semibold mb-2'>
          {t('wallet.dialog.confirm_step.title')}
        </h3>
        <p className='text-sm text-gray-600'>
          {t('wallet.dialog.confirm_step.subtitle')}
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className='text-lg'>
            {t('wallet.dialog.confirm_step.summary')}
          </CardTitle>
        </CardHeader>
        <CardContent className='space-y-4'>
          <div className='flex justify-between'>
            <span className='text-gray-600'>
              {t('wallet.dialog.confirm_step.amount')}
            </span>
          ...
          <div className='flex justify-between'>
            <span className='text-gray-600'>
              {t('wallet.dialog.confirm_step.method')}
            </span>
            <span className='font-semibold'>
              {withdrawalData.paymentMethod === 'bank_transfer'
                ? t('wallet.dialog.method_step.bank_transfer')
                : t('wallet.dialog.method_step.stripe')}
            </span>
          </div>
          {withdrawalData.paymentMethod === 'bank_transfer' && (
            <>
              <div className='flex justify-between'>
                <span className='text-gray-600'>
                  {t('wallet.dialog.confirm_step.holder')}
                </span>
                <span className='font-semibold'>
                  {withdrawalData.bankDetails?.accountHolderName}
                </span>
              </div>
              <div className='flex justify-between'>
                <span className='text-gray-600'>
                  {t('wallet.dialog.details_step.iban')}:
                </span>
                <span className='font-mono text-sm'>
                  {withdrawalData.bankDetails?.iban}
                </span>
              </div>
            </>
          )}
          {withdrawalData.paymentMethod === 'stripe_connect' && (
            <div className='flex justify-between'>
              <span className='text-gray-600'>
                {t('wallet.dialog.confirm_step.stripe_account')}
              </span>
              <span className='font-semibold'>
      ...
      <DialogContent className='max-w-md'>
        ...
        <div className='space-y-6'>
          ...
          <div className='flex justify-center space-x-2'>
            {['amount', 'method', 'details', 'confirmation'].map(
              (stepName, index) => (
                <div
                  key={stepName}
                  className={`w-3 h-3 rounded-full ${
                    step === stepName
                      ? 'bg-primary'
                      : ['amount', 'method', 'details', 'confirmation'].indexOf(
                            step,
                          ) > index
                        ? 'bg-green-500'
                        : 'bg-gray-300'
          ...
          <div className='flex justify-between pt-4'>
            ...
            {step === 'confirmation' ? (
              ...
              >
                {loading
                  ? t('wallet.dialog.buttons.processing')
                  : t('wallet.dialog.buttons.confirm')}
              </Button>
            ) : (
              <Button
                onClick={handleNext}
                disabled={
                  (step === 'amount' &&
                    (!withdrawalData.amount || !!errors.amount)) ||
                  (step === 'amount' &&
                    currency.code !== 'GBP' &&
                    (!ratesReady || isRatesLoading))
                }
              >
