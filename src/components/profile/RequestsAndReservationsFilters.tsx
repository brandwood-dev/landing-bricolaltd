...
const RequestsAndReservationsFilters = ({ 
  ...
  const suggestions = useMemo(() => {
    if (!filters.searchTerm || filters.searchTerm.length < 2 || !data || !Array.isArray(data)) return [];
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.periodFilter !== 'all') {
      ...
      result = result.filter(item => {
        const itemDate = new Date(item.startDate || item.createdAt || Date.now());
  ...
  return (
    <div className="bg-white p-4 rounded-lg border mb-6 space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {/* Period filter */}
        <Select value={filters.periodFilter} onValueChange={(value) => handleFilterChange('periodFilter', value)}>
          <SelectTrigger>
